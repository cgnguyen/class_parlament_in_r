####Setup####
  ####*Installing Germaparl- currently not on Cran- nur einmal####
  ...
  ####* Download voller Corpu für GermaParl - nur einmal notwendig####
...
####*Spezifische Worte Zählen und Darstellen####
  ...
  temp %>%
    ...
    ggplot()+
      ...
      geom_line(size=1)+
      theme_minimal()+
      labs(x="Jahr",y="Frequenz der Wortnutzung", title="Wer spricht über Migrationshintergrund?")+
  ...
  dispersion("GERMAPARL", 
    ...
    ggplot()+
      ...
      geom_line(size=1)+
      theme_minimal()+
      labs(x="Jahr",y="Frequenz der Wortnutzung", title="Wer spricht über Migration?")+
      scale_colour_manual(values = c("Black","Blue","Yellow","Dark Green","Purple","Red"))+
      scale_y_continuous(labels=comma)+
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
